...
module.exports = function buildDockerComposeConfig( config ) {
	...
	const dbImage = config.dbImage || 'mariadb'
	...
	return {
		...
		services: {
			mysql: {
				...
				image: config.env.development.dbImage || 'mariadb',
				ports: [ '3306' ],
				environment: {
					MYSQL_ROOT_PASSWORD: dbEnv.credentials.WORDPRESS_DB_PASSWORD,
					MYSQL_DATABASE: dbEnv.development.WORDPRESS_DB_NAME,
					// MYSQL_USER: dbEnv.credentials.WORDPRESS_DB_USER,
					// MYSQL_PASSWORD: dbEnv.credentials.WORDPRESS_DB_PASSWORD,
				},
				volumes: [ './bin/local-dev/mysql:/var/lib/mysql' ],
				restart: 'always',
			},
			'tests-mysql': {
				platform: 'linux/x86_64',
				image: config.env.tests.dbImage || 'mariadb',
				ports: [ '3306' ],
				environment: {
					MYSQL_ROOT_PASSWORD: dbEnv.credentials.WORDPRESS_DB_PASSWORD,
				...
				volumes: [ './bin/local-dev/mysql-test:/var/lib/mysql' ],
				restart: 'always',
			},
			wordpress: {
				build: '.',
