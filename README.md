...
**-- Adds new configuration option to specify the underlining database image.--**

By default the `mariadb` database image is used for `wp-env`. In most cases this is perfectly fine, this package is for the edge case when it is not.

Override the image by using `.wp-env.json` configuration file and changing the following new option:
...
If you are not sure why you need this project, then this is not for you. Stick to the official project.

---

Below is the same as the official README.md
